export const SKILLS = [
    {
        title: "Frontend",
        icon: "./assets/images/ux.png",
        skills: [
            { skill: "HTML5", percentage: "80%" },
            { skill: "CSS3", percentage: "70%" },
            { skill: "JavaScript", percentage: "80%" },
            { skill: "React.js", percentage: "50%" }
        ]
    },
    {
        title: "Backend",
        icon: "./assets/images/backend.png",
        skills: [
            { skill: "JAVA", percentage: "70%" },
            { skill: "C#", percentage: "70%" },
            { skill: "PHP", percentage: "70%" },
            { skill: "JavaScript", percentage: "80%" }
        ]
    },
    {
        title: "Herramientas",
        icon: "./assets/images/framework.png",
        skills: [
            { skill: "Visual Studio & VS Code", percentage: "80%" },
            { skill: "Git & GitHub", percentage: "80%" },
            { skill: "NetBeans", percentage: "80%" },
            { skill: "IntelliJ IDEA", percentage: "70%" }
        ]
    },
    {
        title: "Soft Skills",
        icon: "./assets/images/competence.png",
        skills: [
            { skill: "Liderazgo", percentage: "90%" },
            { skill: "Resolución de problemas", percentage: "80%" },
            { skill: "Trabajo en equipo", percentage: "85%" },
            { skill: "Comunicación", percentage: "85%" }
        ]
    }
];

export const ACADEMIC_ACTIVITY = [
    {
        title: "Tecnólogo Análisis y Desarrollo de Sistemas de información - SENA",
        date: "Enero 2022 - Enero 2024",
        responsibilities: [
            "Análisis de requerimientos",
            "Levantamiento de información",
            "Desarrollo Web en JAVA, PHP, Node.js, C# (.NET)",
            "Diseño de bases de datos",
            "Administración y manejo de bases de datos SQL",
            "Pruebas unitarias Junit",
            "Calidad de software",
            "Documentación de proyectos",
            "Diseño web",
            "Inglés"
        ]
    },
    {
        title: "Fundamentos profesionales del desarrollo de Software - Microsoft y LinkedIn",
        date: "Abril 2023",
        responsibilities: [
            "Fundamentos esenciales de la programación",
            "Introducción a las habilidades profesionales en el desarrollo de software",
            "Fundamentos de la programación: Más allá de lo básico"
        ]
    },
    {
        title: "Fundamentos profesionales del análisis de datos - Microsoft y LinkedIn",
        date: "Abril 2023",
        responsibilities: [
            "Introducción a las habilidades profesionales en análisis de datos",
            "Aprende análisis de datos: fundamentos",
            "Aprende análisis de datos: Ampliación y aplicación de los conocimientos básicos",
            "Aprende data science: Conceptos básicos",
            "Aprende data science: Cuenta historias con los datos"
        ]
    },
    {
        title: "Lógica de programación con JAVASCRIPT",
        date: "Enero 2024",
        responsibilities: [
            "Orientación a objetivos",
            "Lógica de programación: JavaScript",
            "Lógica de programación: explorar funciones y listas",
            "Git y GitHub: repositorio, commit y versiones"
        ]
    },
    {
        title: "Fundamentos profesionales de IA generativa - Microsoft y LinkedIn",
        date: "Febrero 2024",
        responsibilities: [
            "Qué es la IA generativa",
            "Inteligencia artificial generativa: la evolución de la búsqueda inteligente en Internet",
            "Aumenta tu productividad con Microsoft Bing Chat",
            "Descubre Microsoft 365 Copilot",
            "Ética en la era de la inteligencia artificial generativa",
            "Inteligencia artificial más allá de la ingeniería"
        ]
    },
];
export const WORK_EXPERIENCE = [
    {
        title: "Desarrollador Junior en Pontificia Universidad Javeriana",
        date: "Julio 2023 - Enero 2024",
        responsibilities: [
            "Prácticas de Tecnólogo en Análisis y Desarrollo de Sistemas de Información",
            "Levantamiento de información",
            "Análisis de requerimientos",
            "Desarrollo de app para automatizar el préstamo de equipos de cmputo a docentes para sus clases",
            "Desarrollo web con uso de PHP",
            "Desarrollo de página web mediante Power Apps, Power Automate y SharePoint",
            "Administración de bases de datos SQL",
            "Soporte técnico de manera oportuna y efectiva a usuarios"
        ]
    },
    {
        title: "Desarrollador Junior en Pontificia Universidad Javeriana",
        date: "Julio 2023 - Enero 2024",
        responsibilities: [
            "Prácticas de Tecnólogo en Análisis y Desarrollo de Sistemas de Información",
            "Levantamiento de información",
            "Análisis de requerimientos",
            "Desarrollo de app para automatizar el préstamo de equipos de cómputo a docentes para sus clases",
            "Desarrollo web con uso de PHP",
            "Desarrollo de página web mediante Power Apps, Power Automate y SharePoint",
            "Administración de bases de datos SQL",
            "Soporte técnico de manera oportuna y efectiva a usuarios"
        ]
    },
    {
        title: "Desarrollador Junior en Pontificia Universidad Javeriana",
        date: "Julio 2023 - Enero 2024",
        responsibilities: [
            "Prácticas de Tecnólogo en Análisis y Desarrollo de Sistemas de Información",
            "Levantamiento de información",
            "Análisis de requerimientos",
            "Desarrollo de app para automatizar el préstamo de equipos de cómputo a docentes para sus clases",
            "Desarrollo web con uso de PHP",
            "Desarrollo de página web mediante Power Apps, Power Automate y SharePoint",
            "Administración de bases de datos SQL",
            "Soporte técnico de manera oportuna y efectiva a usuarios"
        ]
    },
];